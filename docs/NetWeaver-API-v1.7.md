...
> 
> Controller 收到 `link_metrics` 后，按 `(source_node_id, target_node_id)` 维度维护延迟时间序列，并用 `link_type` 更新真实边类型，作为 `GET /api/v1/dashboard/nodes/{node_id}/metrics` 和 `GET /api/v1/dashboard/edges` 的真实数据来源。延迟数据来自节点间**主动探测**（如 ICMP ping 或 UDP probe），而非心跳附带采集。
