...
前端控制台当前直接依赖这个接口渲染表格、统计卡片、关系图节点信息和悬浮详情卡片。
...
- **字段说明:**
  ...
  - `public_ip` (string \| null): 节点的公网出口 IP。如果节点处于纯内网环境或未探测到公网 IP，此字段为空 (`null` 或 `""`)。
  - `nat_type` (string): 探测到的 NAT 类型。可选值包括：`Full Cone`, `Restricted Cone`, `Port Restricted Cone`, `Symmetric`, `Unknown`。
  - `status` (string): 节点当前在线状态，仅包含 `online`（在线）或 `offline`（离线）。
  - `connected_peers` (number): 当前与该节点建立 P2P 或 Relay 连接的对等节点数量（表示数量，非对象数组）。
...
获取当前网络中节点间的真实连接关系，用于前端渲染拓扑图的连线。**说明：关系边数据不直接并入 nodes 接口返回里，前端需调用此独立接口进行关系连线。**
...
<!-- -->

    {
      ...
      "data": {
        "edges": [
          {
            "source": "nw-node-a1b2",     // 源节点 node_id
            "target": "nw-node-c3d4",     // 目标节点 node_id
            "type": "p2p"                 // 连线类型，例如 "p2p" 或 "relay"
          }
        ]
      }
    }
...
获取节点间的链路延迟历史数据，用于悬浮详情卡片的折线图渲染。返回数据为时间序列数组。

- **请求方式 & 路径:** `GET /api/v1/dashboard/nodes/{node_id}/metrics`

- **查询参数:**

  - `target_id` (string, 必填): 目标节点 ID，用于查询当前节点到指定对端节点的链路延迟。

  - `time_range` (string, 可选): 查询的时间范围（如 `1h`, `12h`, `24h`），默认值为 `1h`。
...
<!-- -->

    {
      ...
      "data": {
        "metrics": [
          {
            "timestamp": 1714819200,    // 数据点的时间戳 (Unix 秒级)
            "latency_ms": 45.5          // 链路延迟数值 (毫秒)
          },
          {
