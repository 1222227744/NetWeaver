...
| `edges.type` 由谁决定    | 由心跳中 `link_metrics` 的 `link_type` 字段决定。Node 上报时写入真实链路类型，Controller 直接取用，不推断 |
...
Dashboard 用户通过此接口获取 JWT Token，后续所有 Dashboard API 请求必须携带此 Token。
...
| 字段名      | 类型   | 说明                              |
|-------------|--------|-----------------------------------|
| token       | string | JWT Token，后续请求放入 `Authorization: Bearer <token>` Header |
| expires_at  | number | Token 过期时间，Unix 秒级时间戳    |
...
| 场景           | code | msg                                  |
|----------------|------|--------------------------------------|
| 用户名或密码错误 | 401  | `"unauthorized: invalid credentials"` |
| 缺少必填参数    | 400  | `"username and password are required"` |
| Token 过期      | 401  | `"unauthorized: token expired"`       |

> **注意：** Dashboard 登录接口与 Node PSK 鉴权属于**不同权限体系**（见 §8.1）。Dashboard 使用用户名/密码 → JWT；Node 使用预共享密钥 PSK。
...
| public_ip           | string   | 是*  | 公网出口 IP。STUN 成功时必须主动上报；STUN 全部失败时允许为空字符串 `""` |
| public_port         | number   | 是*  | 公网映射端口。STUN 成功时必须主动上报；STUN 全部失败时允许为 `0`         |
...
> **`public_ip` / `public_port` 上报策略：**
> 
> - **STUN 探测成功时**：节点**必须主动上报** `public_ip` 和 `public_port`，Controller 不负责推断。
> - **STUN 探测全部失败时**：`public_ip` 允许为空字符串、`public_port` 允许为 `0`。Controller 仍接受心跳（200），节点保持在线但标记为"无法建 P2P 边"。此时节点只能与其他节点通过 Relay 通信（如果 Relay 可用），或等待 STUN 服务恢复后下一轮心跳更新。
> - 此策略确保：**STUN 服务不可用时系统不会整体瘫痪，节点可降级运行。**
...
```json
{
  ...
  "link_metrics": [
    { "target_node_id": "nw-node-c3d4", "link_type": "p2p", "latency_ms": 42.1 },
    { "target_node_id": "nw-node-e5f6", "link_type": "relay", "latency_ms": 18.7 }
  ]
}
```
...
> | link_type       | string | 是   | 到该邻居的**真实链路类型**：`"p2p"` 或 `"relay"`。Controller 直接取此字段写入 `edges.type` |
...
| relay_session_id   | string | 条件必填 | Controller 分配的 Relay 会话标识。**8 位十六进制字符串**（如 `"a1b2c3d4"`）。数据面头取 `hex → uint32` 作为 Session ID（见 §6.2） |
...
| Flags         | 1 Byte  | bit0: 压缩, bit1: 加密, bit2: Keepalive, bit3: 建链握手 ACK。bit4-bit7 保留 |
| Session ID    | 4 Bytes | 会话标识。P2P 模式为 `0x00000000`。Relay 模式：取 `relay_session_id`（8 位 hex 字符串，如 `"a1b2c3d4"`）→ hex 解码为 uint32 → 大端序写入 4 Bytes |
...
P2P 打洞的最后一步：打洞发起方发送握手请求包，对端回复握手 ACK，至此建链成功。

| 项目           | 规定                                                         |
|----------------|--------------------------------------------------------------|
| 握手请求       | 打洞发起方发送空 Payload 的 UDP 包，NetWeaver Header 中 `Flags.bit3=1`、`Session ID=0` |
| 握手 ACK       | 对端收到后回复相同格式包：`Flags.bit3=1`、`Session ID=0`、Payload 为空 |
| 超时           | 发起方等待 ACK **2 秒**，超时视为本次打洞失败                   |
| 建链成功判定   | 发起方收到对端握手 ACK → 建链成功 → 将 peer 加入 `connected_peer_ids` |

**握手 ACK 报文格式**（与 Keepalive 共用 NetWeaver Header，靠 Flags 区分）：

```
┌──────────────────────────────────────────────┐
│  NetWeaver Header                             │
│  Flags.bit3 = 1 (建链握手 ACK)                 │
│  Session ID = 0 (P2P)                         │
│  Payload Length = 0                           │
├──────────────────────────────────────────────┤
│  (无 Payload)                                  │
...
| 约束项             | 规定值 / 行为                                          |
|--------------------|--------------------------------------------------------|
| Keepalive 触发     | 数据面空闲超过 **30 秒**时，Node 向每个已连接 peer 发送   |
| Keepalive 包格式   | `Flags.bit2=1`，Payload 为空                             |
| Keepalive 响应     | 收到 Keepalive 包后立即回复一个 `Flags.bit2=1`、Payload 为空的包 |

> **注意：** 建链握手 ACK（`Flags.bit3=1`）与 Keepalive（`Flags.bit2=1`）复用同一封装格式，通过 Flags 不同位区分。
...
| STUN 主服务器        | `<host>:<port>`               | STUN 主服务器地址，如 `<stun-primary-host>:3478`（具体地址由部署时配置） |
| STUN 备服务器        | `<host>:<port>`               | STUN 备服务器地址（NAT 类型交叉判定用）。如部署仅有一个 STUN 端点，此值可与主服务器相同 |
...
> **注意：** 如果 STUN 探测全部失败，仍必须发送心跳，`nat_type` 为 `"Unknown"`，`public_ip` 为空字符串，`public_port` 为 `0`。Controller **接受此心跳（200）**，节点保持在线但处于"无法建 P2P 边"的降级状态。STUN 服务恢复后，下一次心跳自动更新 NAT 信息。详见 §3.2 上报策略。
