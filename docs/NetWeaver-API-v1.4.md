...
**本文档仅覆盖 HTTP 控制面接口**，具体包括：

| 通信方向          | 协议     | 说明                                 |
|-------------------|----------|--------------------------------------|
| Dashboard ↔ Controller | HTTP/JSON | 管理控制台查询网络状态、拓扑、链路数据 |
| Node ↔ Controller      | HTTP/JSON | 节点注册、心跳保活、Peer 列表拉取     |

**本文档不覆盖以下协议**，它们由独立的正式文档定义：

| 协议层       | 文档名                       | 说明                                  |
|--------------|------------------------------|---------------------------------------|
| Node ↔ Node 数据面 | `NetWeaver-DataPlane-Spec.md`   | P2P 直连与 Relay 转发的 TUN 报文封装格式 |
| Node ↔ Relay 数据面 | `NetWeaver-DataPlane-Spec.md`   | Relay 转发时的封装格式、会话标识与保活     |
| STUN 探测协议       | `NetWeaver-DataPlane-Spec.md`   | NAT 类型探测的 STUN 交互流程            |
| 建链信令协议        | 本文档 §5（建链协议）           | P2P 打洞 / Relay 回退的状态机与 Controller 协同 |

> **阅读指引：** 如果你只需要对接 Controller 的 HTTP API（如开发 Dashboard 或实现 Node 注册/心跳逻辑），本文档即完整。如果你需要实现 TUN 数据转发、Relay 服务或 P2P 打洞逻辑，请同时参阅 `NetWeaver-DataPlane-Spec.md`。
...
本章定义 TUN 报文在 Node ↔ Node（P2P）和 Node ↔ Relay 链路上的封装格式。**详细字段定义见 `NetWeaver-DataPlane-Spec.md`**，此处仅给出最小必须约定。
...
| 约束项             | 规定值 / 行为                                          |
|--------------------|--------------------------------------------------------|
| MTU 约束           | TUN 设备 MTU 建议 **1400 Bytes**（预留外层 IP+UDP+Header 开销）。实际路径 MTU 由 PMTUD 动态发现 |
| Keepalive 包       | 数据面空闲超过 **30 秒**时，Node 向每个已连接 peer 发送 Keepalive 包（`Flags.bit2=1`，Payload 为空） |
| 目标节点识别       | 由 NetWeaver Header 中的 `Dst Virtual IP` 字段识别，Relay 服务器根据此字段查表转发 |
| Relay 复用封装     | Relay 转发时**使用相同封装格式**，区别仅在于 `Session ID` 字段为 relay 会话 ID 而非 0 |
...
| STUN 服务器地址        | `<host>:<port>`               | STUN 服务器地址，如 `stun.netweaver.io:3478`       |
...
| 传输加密               | 控制面使用 HTTPS（生产环境强制）。数据面加密见 `NetWeaver-DataPlane-Spec.md` |
...
| v1.4  | 2026-05    | 文档范围声明；metrics 完整语义；状态判定规则；心跳上报规则；Relay 支撑；建链协议；数据面封装；运行配置；安全约定；平台边界；验收标准 |
