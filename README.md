...
本次新增一轮“偏实际落地”的收口，重点不是继续堆功能，而是把已经有的功能改得更像可演示、可联调、可部署的控制台。

同时补上了一套服务器一键部署脚本：

- `scripts/deploy_server.sh` 会在 Linux 服务器上完成后端测试、后端编译、前端构建、环境模板生成和控制台静态文件准备
- `scripts/install_systemd.sh` 会把生成的 systemd unit 安装到服务器上，便于常驻运行
- controller 已支持可选托管前端静态页面，部署后可直接通过同域名访问 `/console/`

这意味着：以后如果你们要在服务器上落地，流程可以收敛成“拉仓库 -> 执行部署脚本 -> 编辑环境文件 -> 启动服务”，不再需要手工拼多个静态服务器或临时预览命令。

本次分支：

```text
feature/fronted-1222227744-落地微调与控制台优化
```
...
- 前端主体布局整理为“概览、拓扑、节点列表”三层结构，信息分层更清楚
- 前端移除全局鼠标光晕，减少展示性视觉干扰
